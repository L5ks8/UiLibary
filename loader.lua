...
if not (success1 and success2) then
    error("GoonHub Loader: Failed to fetch core files from GitHub.")
end
...
local components = {}
local componentNames = {"button", "toggle", "slider", "status", "paragraph", "dropdown", "avatar"}
...
for _, name in ipairs(componentNames) do
    ...
    if success then
        ...
        error("GoonHub Loader: Failed to fetch component '" .. name .. "' from GitHub.")
    end
end
